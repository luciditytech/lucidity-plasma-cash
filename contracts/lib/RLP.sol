...
library RLP {
  ...
  function toRLPItem(bytes memory self, bool strict) internal pure returns (RLPItem memory) {
    ...
    if (strict) {
      ...
      if (_payloadOffset(item) > len)
        revert("revert toRLPItem(1)");
      if (_itemLength(item._unsafe_memPtr) != len)
        revert("revert toRLPItem(2)");
      if (!_validate(item))
        revert("revert toRLPItem(3)");
    }
    return item;
  }
  ...
  function toBool(RLPItem memory self) internal pure returns (bool data) {
    ...
    if (len != 1)
      revert("revert toBool(2)");
    uint temp;
    assembly {
      temp := byte(0, mload(rStartPos))
    }
    if (temp > 1)
      revert("revert toBool(3)");
  ...
  returns (byte data)
  {
    require(isData(self), "isData(self)");
    uint rStartPos;
    uint len;
    (rStartPos, len) = _decode(self);
    if (len != 1)
      revert("revert toByte()");
    uint temp;
    assembly {
      temp := byte(0, mload(rStartPos))
    }
